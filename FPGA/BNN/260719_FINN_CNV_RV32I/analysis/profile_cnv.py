...
def load_symbols(elf: Path) -> dict[str, tuple[int, int]]:
    text = subprocess.check_output(["riscv32-unknown-elf-nm", "-S", "--defined-only", str(elf)], text=True)
    result = {}
...
def main() -> None:
    ...
    p = load_module(args.profiler)
    cpu = p.RV32I(p.load_verilog_words(args.imem), p.load_verilog_words(args.dmem), dmem_size=1 << 20)
    sym = load_symbols(args.elf)
    ...
    for wanted in ("xnor_word", "conv1_q7", "binary_conv3x3", "binary_fc", "finn_cnv_inference", "main"):
        name = next(key for key in sym if key == wanted or key.startswith(wanted + "."))
        start, size = sym[name]
        dynamic = sum(count for pc, count in pc_counts.items() if start <= pc < start + size)
        print(f"PROFILE function_{wanted}={dynamic}")
    ...
    print(f"PROFILE layer_checks_ok={cpu.read32(0x20000010)}")
    print("PROFILE scores=" + ",".join(str(p.sext(cpu.read32(0x20000040 + 4*i), 32)) for i in range(10)))
