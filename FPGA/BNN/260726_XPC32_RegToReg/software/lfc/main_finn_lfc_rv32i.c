...
__attribute__((noinline)) static uint32_t xnor_match32(
...
#ifdef USE_XPC32
    /*
     * XPC32 always counts all 32 bits. Process every full word with XPC32 and
     * retain the masked software operation only for a partial final word.
     */
    uint32_t full_words = words;
    if ((words != 0u) && (tail_mask != 0xffffffffu)) {
        full_words = words - 1u;
    }

    for (uint32_t index = 0; index < full_words; ++index) {
        matches += xpc32(activation[index], weight[index]);
    }

    if (full_words != words) {
        const uint32_t equal =
            ~(activation[full_words] ^ weight[full_words]) & tail_mask;
    ...
    uint32_t* output)
{
    ...
    const uint32_t tail_mask =
        tail_bits ? ((1u << tail_bits) - 1u) : 0xffffffffu;
    ...
    for (uint32_t neuron = 0; neuron < output_bits; ++neuron) {
        const uint32_t matches =
            xnor_match32(input, weights + neuron * words, words, tail_mask);
        const uint32_t ge = matches >= thresholds[neuron];
        const uint32_t polarity =
            (polarities[neuron >> 5] >> (neuron & 31u)) & 1u;
        if (ge == polarity) {
...
__attribute__((noinline)) static int finn_lfc_inference(int32_t scores[10])
{
    ...
    binary_threshold_layer(
        lfc_input, lfc_w0, lfc_threshold0, lfc_polarity0,
        784, 1024, activation_a);
    binary_threshold_layer(
        activation_a, lfc_w1, lfc_threshold1, lfc_polarity1,
        1024, 1024, activation_b);
    binary_threshold_layer(
        activation_b, lfc_w2, lfc_threshold2, lfc_polarity2,
    ...
    for (uint32_t neuron = 0; neuron < 10; ++neuron) {
        const uint32_t matches =
            xnor_match32(
                activation_a, lfc_w3 + neuron * 32u,
                32, 0xffffffffu);
