...
module rv32i_cnv_bdot_tb;
    ...
    task finish_and_check;
        begin
            $display("CNV BDOT result cycles=%0d bdot=%0d blocks=%0d bcfg=%0d errors=%0d",
                     cpu_cycles, bdot_count, block_read_count, bcfg_count, error_count);
            $display("CNV BDOT status=%0d prediction=%0d expected=%0d correct=%0d checks=%0d",
                     dmem[0], dmem[1], dmem[2], dmem[3], dmem[4]);
            $display("CNV BDOT scores=%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0d",
                ...
                $signed(dmem[25]));
            $display("CNV BDOT checksums=%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x",
                dmem[32], dmem[33], dmem[34], dmem[35], dmem[36],
                dmem[37], dmem[38], dmem[39], dmem[40], dmem[41]);
            if ((dmem[0] != 1) || (dmem[1] != 3) || (dmem[2] != 3) ||
                (dmem[3] != 1) || (dmem[4] != 1) ||
                (bdot_count != 756746) || (block_read_count != 761128) ||
                (bcfg_count != 8) || (error_count != 0) ||
                ($signed(dmem[16]) != -40) || ($signed(dmem[17]) != -46) ||
                ($signed(dmem[18]) != -32) || ($signed(dmem[19]) != 404) ||
                ($signed(dmem[20]) != -30) || ($signed(dmem[21]) != 24) ||
                ($signed(dmem[22]) != -14) || ($signed(dmem[23]) != -28) ||
                ($signed(dmem[24]) != -16) || ($signed(dmem[25]) != -42)) begin
                $display("TB FAIL: FINN CNV BDOT mismatch");
                $fatal(1);
            end
            $display("TB PASS: FINN CNV BDOT128");
            $finish;
        end
    endtask
