...
BUILD_DIR="${SCRIPT_DIR}/build/wide_bdot_accel"

mkdir -p "${BUILD_DIR}"

iverilog -g2012 -Wall \
  -s wide_bdot_accel_tb \
  -o "${BUILD_DIR}/wide_bdot_accel_tb.out" \
  ...
  "${SCRIPT_DIR}/wide_bdot_accel_tb.v"

vvp "${BUILD_DIR}/wide_bdot_accel_tb.out"
