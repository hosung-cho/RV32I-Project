...
BUILD_DIR="${PROJECT_DIR}/build/verilator_ebnn_bdot"
SOFTWARE_DIR="${PROJECT_DIR}/software/ebnn"

"${SOFTWARE_DIR}/build_ebnn_bdot.sh"
mkdir -p "${BUILD_DIR}"

CCACHE_DISABLE=1 verilator --binary --timing -Wno-fatal \
  --top-module rv32i_ebnn_bdot_tb --Mdir "${BUILD_DIR}" \
  ...
  "${PROJECT_DIR}/testbench/rv32i_ebnn_bdot_tb.v"

"${BUILD_DIR}/Vrv32i_ebnn_bdot_tb" \
